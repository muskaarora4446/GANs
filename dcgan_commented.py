...
#to make it compatible with the NN of the generator. 

# Loading the dataset
dataset = dset.CIFAR10(root = './data', download = True, transform = transform) # We download the training set in the ./data folder and we apply the previous transformations on each image.
dataloader = torch.utils.data.DataLoader(dataset, batch_size = batchSize, shuffle = True, num_workers = 2) # We use dataLoader to get the images of the training set batch by batch.
#shuffle = true means can load the data in random order, num_workers = 2 means we are going to hav etwo parallel threads that will load the data. using these two we get the dataset loaded much faster when dataset is very huge, which is case of cifar10 data.
...
def weights_init(m):
    ...
    if classname.find('Conv') != -1:
        ...
        m.bias.data.fill_(0)
#we will apply this fuction to both the NN of generator and discriminator to initial all their weights in the right way for the adversarial networks.

# Defining the generator

#we are going to inherit NN.nodules becoz it conatain all the imp tools to build an NN.
class G(nn.Module): # We introduce a class to define the generator.

    def __init__(self): # We introduce the __init__() function that will define the architecture of the generator.
        super(G, self).__init__() # We inherit from the nn.Module tools. to activate the inherit we use super() function.
        
        #we will create a meta module. A meta module is a sequence of several modules, some covolutions, full connections and inversed convolutions.
        #main will be meta module that will contain all the different modules in a sequence of layers.
        self.main = nn.Sequential( # We create a meta module of a neural network that will contain a sequence of modules (convolutions, full connections, etc.).
            #we will start with inverse CNN becoz a normal CNN takes image as an input and returns its output vector. with in GANS we input a vector(that we call noise in the tutorial) and it will give us an output of fake images.
            #size of random vector = 100, no of feature maps, size of the kernel(4X4), stride, padding.
            ...
            nn.Tanh() # We apply a Tanh rectification to break the linearity and stay between -1 and +1 and centered around zero. becoz not out of generator will go as input to the discriminator class.
        )
    ...
    def forward(self, input): # We define the forward function that takes as argument an input that will be fed to the neural network, and that will return the output containing the generated images.
        #input that will be called random noise vector.
...
class D(nn.Module): # We introduce a class to define the discriminator.
    ...
    def __init__(self): # We introduce the __init__() function that will define the architecture of the discriminator.
        ...
        self.main = nn.Sequential( # We create a meta module of a neural network that will contain a sequence of modules (convolutions, full connections, etc.).
            #it will take an image as an input and return a single vector of one element i.e number between 0 and 1. therefore, we will use an CNN. becoz takes image as an input.
    ...
    def forward(self, input): # We define the forward function that takes as argument an input that will be fed to the neural network, and that will return the output which will be a value between 0 and 1.
        ...
        #we want to flatten our results of convolution that are in the 2d into one same dimension along one same flattened vector.
...
#we created the brain of the NN becoz brain consists of both the generator and discriminator.
#now we hav created the brain and now we are going to train the brain.

# Training the DCGANs

criterion = nn.BCELoss() # We create a criterion object that will measure the error between the prediction and the target using binary cross entropy 
...
for epoch in range(25): # We iterate over 25 epochs.
    ...
    for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset.
        ...
        # the first element is dat and second element is labels. data is set of mini batches
        real, _ = data # We get a real image of the dataset which will be used to train the discriminator.
        #not in minibatch but also a torch variable. so, now we are allowed to feed this NN with this input.
        input = Variable(real) # We wrap it in a variable.
        #since we are having ral image of dataset here.Therefore, our target is 1.so, ground truth is one.
        ...
        #input should be mini batch of fake images.
        #batch size, vector of size 100 (becoz made NN with that)/no of elements, each of the 100 feature map will be a matrix of size 1X1.
        ...
        target = Variable(torch.zeros(input.size()[0])) # We get the target. we want asa many ones as there are number of images in a mini batch.
        #we going to detach() the gradient of this fake torch variable taht will save up some memory and speed up the computations.
        ...
        target = Variable(torch.ones(input.size()[0])) # We get the target.
        #we need the gradient of fake becoz this time we are going to update the weights of the neural network.
